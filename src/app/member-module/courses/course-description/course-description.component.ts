...
export class CourseDescriptionComponent implements OnInit {
  ...
  openDialog(): void {
    const dialogRef = this.dialog.open(DialogOverviewEnrollMember, {
      ...
      data: {
        ...
        subscription_membership: this.course.subscription_membership,
        session_membership: this.course.session_membership
      }
    });
  ...
  removeStudent() {
    if (window.confirm("Are you sure you want to unenroll from this course?")) {
      this.courseApi
        .RemoveStudent(this.course_id, { member_id: this.value })
        .subscribe(res => {
...
export class DialogOverviewEnrollMember {
    ...
    this.courseApi.GetCourse(this.course_id).subscribe(data => {
      ...
      userApi.GetUser(this.value).subscribe(data2 => {
        ...
        var timeDiff = Math.abs(
          Date.now() - new Date(data2.birthdate).getTime()
        );
    ...
    this.courseApi.GetCourse(this.course_id).subscribe(data => {
      ...
      if (data.session_membership != null) {
        this.memberships.push({
          key: "Session",
          type: data.session_membership._id
        });
      }
      if (data.subscription_membership != null) {
        this.memberships.push({
          key: "Subscription",
          type: data.subscription_membership._id
        });
      }
  ...
  enrollMember() {
    if (this.courseForm.valid) {
      if (
        window.confirm(
          "Are you sure you want to enroll this member to the course?"
        )
      ) {
        if (this.studCount >= this.course.max_students) {
          ...
          if (this.age > this.maxAge || this.age < this.minAge) {
            ...
            this.courseApi
              .EnrolMember(this.course_id, {
                member_id: this.value,
                membership_id: this.membership_selected
              })
              .subscribe(res => {
                this.closeDialog();
                this.ngZone.run(() =>
                  this.router.navigateByUrl("/member/courses")
                );
              });
          }
        }
