...
sessionRoute.route('/add-session').post((req, res, next) => {
  ...
  Session.create(req.body, (error, data) => {
    ...
    //closes the session after a certain time
    setTimeout(() => {
      Session.findByIdAndUpdate(data._id, {
        $set: {"open": false}
      }, (error, data) => {
    ...
    //log the event
    Log.newLog('started a new session')
  ...
  User.findById(req.body.subject)
    ...
    .then(
      function(membershipData){
        ...
        //log the event
        Log.newLog("member has checked-in",req.body.subject)
      }
    )
...
sessionRoute.route('/close-session/:id').put((req, res, next) => {
  Session.findByIdAndUpdate(req.params.id, {
    ...
    if (error) {
      ...
      console.log('Session closed!')
      //log the event
      Log.newLog("Session closed. Session id: "+data._id)
      res.json(data)
    }
  })
})
